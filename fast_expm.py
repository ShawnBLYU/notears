...
import numpy as np
def fast_expm(W1, W0=None, expm0=None, threshold=1e-5, num_terms=20, debug=False):
  if W0 is None and expm0 is None:
    return(slin.expm(W1))
  elif (W0 is None and expm0 is not None) or (W0 is not None and expm0 is None):
    raise SyntaxWarning("Please specify both original matrix and original exponential if fast exponentiation is wanted.")
    return(slin.expm(W1))
